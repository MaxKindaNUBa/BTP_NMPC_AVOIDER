...
N_wave = np.array(scipy.io.loadmat(wave_data_path +"DN_SALV_REV1.mat")['DN'])
...
def MMG_Time_Derivative(u,v,r,x,y,psi,delta,rps,WHA,w_flag):
    ...
    if w_flag == True:
        ...
        fA1 = interpolate.RegularGridInterpolator((hangle_drift, wave_drift), A1.T, bounds_error=False, fill_value=None)
        fA2 = interpolate.RegularGridInterpolator((hangle_drift, wave_drift), A2.T, bounds_error=False, fill_value=None)
        fA3 = interpolate.RegularGridInterpolator((hangle_drift, wave_drift), A3.T, bounds_error=False, fill_value=None)

        # Group the coordinates into a list [x, y]
    ...
    # print(X[0].shape,X[1].shape,X[2].shape,Vel_Mom[0].shape,Vel_Mom[1].shape,Vel_Mom[2].shape)
...
def activateMMG(ip,delta_d,W_flag,WHA,RuPos,h,rps = 18.2):
    """
    ----------
    ip      : current state of the ship [u,v,r,x,y,psi]
    ...
    # print(u, v, r, x, y, psi, delta, rps, WHA, W_flag)
    ...
    return nxt_state.tolist(), r_dot_a


'''# --- Simulation Setup ---
sim_time = 200
delta_t = 0.05
time = np.arange(0, sim_time, delta_t)
rudder_angle_deg = 15
rudder_angle_rad = np.deg2rad(rudder_angle_deg)
...
# # Second Order
# son_ip = [0.78, 0, 0, 0, 0, 0]  # [u, v, r, x, y, psi]
# son_r_dot = 0
# son_r = [son_ip[2]]

# MMG
mmg_ip = [0.78, 0, 0, 0, 0, 0]  # [u, v, r, x, y, psi]
mmg_r = [mmg_ip[2]]

coordinates = [[],[]]
# --- Run Simulation ---
for t in time:
#     # First Order Nomoto Model
#     r_new, psi_new, x_new, y_new = FON_dynamics(fon_x, fon_y, fon_psi, fon_r, rudder_angle_rad)
#     fon_r = r_new.tolist()
#     fon_psi = psi_new.tolist()
#     fon_x = x_new.tolist()
#     fon_y = y_new.tolist()

#     # Second Order Model
#     son_ip, son_r_dot = SON_dynamics(son_ip, 0, rudder_angle_rad, son_r_dot, h=0.05)
#     son_r.append(son_ip[2])
# MMG Model
    mmg_ip, _ = activateMMG(mmg_ip, 0, False, 0, rudder_angle_rad,h=0.05)
    coordinates[0].append(mmg_ip[3])
    coordinates[1].append(mmg_ip[4])
    mmg_r.append(mmg_ip[2])


# --- Plotting ---
plt.figure(figsize=(12, 8))

# Yaw Rate Comparison for Turning Circle Experiment
plt.subplot(1, 2, 1)
plt.plot(np.arange(0, sim_time + delta_t, delta_t), mmg_r, label='MMG Model', linewidth=2, linestyle='-.')

plt.title('Yaw Rate Comparison', fontsize=16)
plt.xlabel('Time (s)', fontsize=12)
plt.ylabel('Yaw Rate (rad/s)', fontsize=12)
plt.legend(fontsize=12)
plt.grid(True)

# Position Plot for Turning Circle Experiment
plt.subplot(1, 2, 2)
plt.plot(coordinates[0], coordinates[1], label='Ship Trajectory', linewidth=2, linestyle='-.')

plt.title('Ship Trajectory', fontsize=16)
...
# for i in range(N):
#     if  i <= 0:
#         temp, _ = activateMMG(data[-1],0,FF, WHA,0.6)
#     elif i > 0:
#         temp, _ = activateMMG(data[-1],0,FF, WHA,0.6)
#     data.append(temp)
#     x1.append(temp[3]/7)
...
#     if  j <= 0:
#         temp1, _ = activateMMG(data1[-1],-0,FF, WHA,-0.6)
#     elif j > 0:
#         temp1, _ = activateMMG(data1[-1],-0,FF, WHA,-0.6)
